...
def should_trigger_summary(
    ...
    """当滑动窗口外新增满 threshold 轮历史时触发一次增量摘要。

    例如 window_size=10、threshold=10：
      - 第 20 轮完成后压缩第 1~10 轮；
      - 第 30 轮完成后压缩第 11~20 轮；
      - 最近 10 轮始终保留原始消息，不参与摘要。
    """
    dialogue_count = sum(
        1 for m in messages if isinstance(m, (HumanMessage, AIMessage))
    )
    completed_rounds = dialogue_count // 2
    archived_rounds = completed_rounds - window_size
    return (
        dialogue_count % 2 == 0
        and archived_rounds >= threshold
        and archived_rounds % threshold == 0
    )
...
def get_summary_batch(
    ...
    """返回本次应合并进摘要的那一批旧消息，不包含最近滑动窗口。

    该函数依赖固定批次节奏：每当窗口外新增 threshold 轮时，仅取最新移出
    活跃窗口的 threshold 轮。旧摘要由 compress_to_summary(existing_summary=...)
    继续承接，因此不会重复压缩更早的原始消息。
    """
    dialogue_messages = [
        m for m in messages if isinstance(m, (HumanMessage, AIMessage))
    ]
    if not should_trigger_summary(
        dialogue_messages, threshold=threshold, window_size=window_size
    ):
        return []

    window_messages = window_size * 2
    batch_messages = threshold * 2
    archive_end = len(dialogue_messages) - window_messages
    archive_start = max(0, archive_end - batch_messages)
    return dialogue_messages[archive_start:archive_end]
